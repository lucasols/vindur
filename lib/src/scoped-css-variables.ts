...
export type ScopedVariableMap = Map<string, { index: number; originalName: string }>;
...
export function processScopedCssVariables(
  ...
  let processedCss = css.replace(declarationRegex, (_match: string, varName: string, value: string) => {
    ...
    let varInfo = existingScopedVariables?.get(trimmedVarName) || scopedVariables.get(trimmedVarName);
    ...
    if (!varInfo) {
      ...
      if (classIndexRef) {
        ...
        const newIndex = Math.max(
          ...
          ...(existingScopedVariables ? [...existingScopedVariables.values()].map(v => v.index) : [])
        ) + 1;
        varInfo = {
          index: newIndex,
          originalName: trimmedVarName,
  ...
  processedCss = processedCss.replace(usageRegex, (_match: string, varName: string, fallback?: string) => {
    ...
    return fallback ? `var(${scopedVarName}, ${fallback})` : `var(${scopedVarName})`;
  });
