...
describe('createDynamicCssColor', () => {
  ...
  test('should transform dynamic color setProps method', async () => {
    const source = dedent`
      import { createDynamicCssColor } from 'vindur'
      ...
      const Component = () => {
        return (
          <div {...color.setProps('#ff6b6b')}>
            <span>Dynamic color content</span>
          </div>
        );
      }
    `;
  ...
  test('should transform dynamic color with mixed self and container selectors', async () => {
    const source = dedent`
      import { createDynamicCssColor, styled } from 'vindur'
  ...
  test('should handle multiple dynamic colors in same component', async () => {
    const source = dedent`
      import { createDynamicCssColor, styled } from 'vindur'
    ...
    expect(result.code).toMatchInlineSnapshot(`
      ...
      const Component = () => {
        return (
          <div dynamicColor={[primaryColor, secondaryColor]}>
            <div className="v1560qbr-3">Multiple colors</div>
          </div>
        );
      };
      "
  ...
  test('should throw error when any argument is passed to createDynamicCssColor', async () => {
    const source = dedent`
      import { createDynamicCssColor } from 'vindur'

      const color = createDynamicCssColor('some-arg')
    `;
