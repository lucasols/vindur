...
describe('compile-time functions should throw at runtime', () => {
  ...
  test('css should throw error at runtime', () => {
    expect(() => css`color: red`).toThrow('css cannot be called at runtime');
  });

  test('createGlobalStyle should throw error at runtime', () => {
    expect(() => createGlobalStyle`body { margin: 0 }`).toThrow('createGlobalStyle cannot be called at runtime');
  });

  test('keyframes should throw error at runtime', () => {
    expect(() => keyframes`from { opacity: 0 } to { opacity: 1 }`).toThrow('keyframes cannot be called at runtime');
  });

  test('styled should throw error at runtime', () => {
    expect(() => styled.div`color: blue`).toThrow('styled cannot be called at runtime');
  });

  test('styled with custom element should throw error at runtime', () => {
    expect(() => styled.button`background: red`).toThrow('styled cannot be called at runtime');
  });
...
describe('styled proxy behavior', () => {
  test('should handle dynamic property access', () => {
    ...
    expect(() => styled.span`color: green`).toThrow('styled cannot be called at runtime');
    expect(() => styled.article`padding: 10px`).toThrow('styled cannot be called at runtime');
    expect(() => styled.section`margin: 5px`).toThrow('styled cannot be called at runtime');
  });
...
describe('function signatures and types', () => {
  test('vindurFn should accept generic function types', () => {
    ...
    const multiFn = (color: string, size: number) => `color: ${color}; font-size: ${size}px`;
  ...
  test('css should accept template literal arguments', () => {
    ...
    expect(() => css`color: ${color}; font-size: ${size}px`).toThrow();
  });
