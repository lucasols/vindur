...
describe('createDynamicCssColor', () => {
  ...
  test('should create color object with proper methods in dev mode', () => {
    ...
    expect(color.alpha(0.5)).toBe(
      'color-mix(in srgb, var(--test-hash) 50%, transparent)',
    );
    expect(color.darker(0.2)).toBe(
      'color-mix(in srgb, var(--test-hash) 80%, #000)',
    );
    expect(color.lighter(0.3)).toBe(
      'color-mix(in srgb, var(--test-hash) 70%, #fff)',
    );
    expect(color.saturatedDarker(0.4)).toBe(
      'color-mix(in srgb, var(--test-hash) 60%, hsl(from var(--test-hash) h 100% 20%))',
    );

    expect(color.contrast.var).toBe('var(--test-hash-c)');
    expect(color.contrast.optimal()).toBe('var(--test-hash-c-optimal)');
    expect(color.contrast.optimal({ alpha: 0.8 })).toBe(
      'color-mix(in srgb, var(--test-hash-c-optimal) 80%, transparent)',
    );
    expect(color.contrast.alpha(0.6)).toBe(
      'color-mix(in srgb, var(--test-hash-c) 60%, transparent)',
    );
  });
  ...
  test('should generate dev-mode class names', () => {
    ...
    expect(color.container.isNotDefined).toBe(
      '.test-hash-container-is-not-defined &',
    );
  });
  ...
  test('should handle set method with warning', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      'color.set() should not be called at runtime',
    );
...
describe('cx', () => {
  ...
  test('should filter out falsy values', () => {
    expect(cx('class1', null, false, undefined, '', 'class2')).toBe(
      'class1 class2',
    );
  });
...
describe('mergeClassNames', () => {
  ...
  test('should handle mixed props', () => {
    ...
    expect(result).toBe('another-string vindur-class');
  });
  ...
  test('should ignore non-string className properties', () => {
    const result = mergeClassNames(
      [{ className: 123 }, { className: null }, { className: 'valid-class' }],
      'vindur-class',
    );
...
describe('mergeStyles', () => {
  ...
  test('should handle props without style property', () => {
    const result = mergeStyles([{ className: 'test' }, { id: 'test-id' }]);
...
describe('createClassName', () => {
  ...
  test('should throw error without id', () => {
    expect(() => createClassName()).toThrow(
      'createClassName cannot be called at runtime',
    );
  });
});
