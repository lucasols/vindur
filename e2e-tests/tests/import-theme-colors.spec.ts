...
test.beforeAll(async ({ browser }) => {
  ...
  env = await startEnvProd('import-theme-colors-prod-tests', {
    ...
    'App.tsx': dedent`
      ...
      if (typeof brandColors.primary.var !== 'string') {
        throw new Error('brandColors.primary.var is not a string');
      }
...
test('should handle alpha colors directly in production mode', async () => {
  ...
  const borderColor = await dangerButton.evaluate(
...
test('should handle multiple imported theme color objects in production mode', async () => {
  ...
  const primaryStyle = await primaryButton.evaluate(
    (el) => getComputedStyle(el).backgroundColor,
  );
  const cardStyle = await successCard.evaluate(
    (el) => getComputedStyle(el).backgroundColor,
  );
  const borderStyle = await successCard.evaluate(
...
test('should handle hover states with inlined colors in production mode', async () => {
  ...
  const hoverBg = await primaryButton.evaluate(
    (el) => getComputedStyle(el).backgroundColor,
  );
